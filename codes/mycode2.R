...
{
  dnorm((t(rbm_w) %*% hidden_state),0,1)
}
...
{
  v0 = dnorm(visible_data,0,1)
  Ph0 = visible_state_to_hidden_probabilities(rbm_w, v0)
  H0 = sample_bernoulli(Ph0)
  vh0 = configuration_goodness_gradient(v0, Ph0)
  V1 = hidden_state_to_visible_probabilities(rbm_w, H0)
  Ph1 = visible_state_to_hidden_probabilities(rbm_w, V1)
  vh1 = configuration_goodness_gradient(V1, Ph1)
  vh0-vh1
}

#  rbm trains a model that's defined by a single matrix of weights.
#	<num_hidden> is the number of hidden units
#	cd1 is a function that takes parameters <model> and <data> and returns the gradient 
...
rbm <- function(num_hidden, training_data, learning_rate, n_iterations, mini_batch_size=100, momentum=0.9, quiet=FALSE)
{
  ...
  for (iteration_number in 1:n_iterations)
  {
    ...
    new_model = model + momentum_speed * learning_rate
    convergence = new_model
  }
  return(model)
}